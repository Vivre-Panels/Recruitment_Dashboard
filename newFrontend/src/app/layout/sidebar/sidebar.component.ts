...
@Component({
  ...
  template: `
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto py-5 px-3 space-y-2 custom-sidebar-scroll">
        <a
          *ngFor="let module of navModules"
          ...
          class="flex items-center justify-between px-3.5 py-3 rounded-xl text-xs font-semibold text-slate-400 hover:text-slate-100 hover:bg-slate-800/70 transition-all duration-150 group cursor-pointer"
          [ngClass]="{ 'justify-center': isCollapsed }"
          [title]="isCollapsed ? module.label : ''"
        >
          <div class="flex items-center gap-3 min-w-0">
            <app-icon [name]="module.icon" [size]="18" class="text-slate-400 group-hover:text-brand-400 transition-colors shrink-0"></app-icon>
      ...
      <div class="p-3 border-t border-slate-800 bg-slate-950/60">
        <div class="flex items-center gap-3 px-2.5 py-2 rounded-xl bg-slate-800/60 border border-slate-700/60 shadow-2xs">
          <div class="relative">
          ...
          <div *ngIf="!isCollapsed" class="min-w-0 flex-1">
            <p class="text-xs font-bold text-slate-100 truncate">Rahul Sharma</p>
            <p class="text-[10px] text-brand-400 font-mono font-semibold">Lead Recruiter (92.4)</p>
          </div>
        </div>
      </div>
    </aside>
